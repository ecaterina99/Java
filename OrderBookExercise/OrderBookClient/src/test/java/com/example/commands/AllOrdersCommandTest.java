...
public class AllOrdersCommandTest {
    ...
    @Test
    void testExecutePrintsSortedOrdersByDistance() throws Exception {
        ...
        try (MockedStatic<FXCalculator> mockCalc = mockStatic(FXCalculator.class)) {
            mockCalc.when(() -> FXCalculator.calculateDistance(rates, o1)).thenReturn(0.5);
            mockCalc.when(() -> FXCalculator.calculateDistance(rates, o2)).thenReturn(0.2);
        ...
        int indexId2 = out.indexOf("id:2");
        int indexId1 = out.indexOf("id:1");
        assertTrue(indexId2 < indexId1, "Order 2 should appear first in output");

        assertTrue(out.contains("BUY") || out.contains("SELL"));
    }

    @Test
    void testSortingByInvestmentCurrency() throws Exception {
        CreateOrderResponseDTO o1 = order("1", true, "EUR", "USD", 1.10);
        CreateOrderResponseDTO o2 = order("2", false, "USD", "EUR", 1.20);

        List<FXRateDTO> rates = List.of(rate("EUR", "USD", 1.11, 1.10));
        ...
        String out = output.toString();
        int index1 = out.indexOf("id:1");
        int index2 = out.indexOf("id:2");

        assertTrue(index1 < index2, "Order with EUR should appear before USD");

    }

    @Test
    void testSortingByCounterCurrency() throws Exception {
        CreateOrderResponseDTO o1 = order("1", true, "USD", "CHF", 1.10);
        CreateOrderResponseDTO o2 = order("2", false, "USD", "EUR", 1.20);

        List<FXRateDTO> rates = List.of(rate("USD", "CHF", 1.11, 1.10));
        ...
        assertTrue(index1 < index2, "Order with CHF should appear before EUR");
    }
}
